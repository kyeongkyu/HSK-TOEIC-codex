...
export default function Navigation() {
  ...
  const showNav = (appMode === 'hsk' || appMode === 'toeic' || appMode === 'jlpt') && isNavigablePage;
  ...
  return (
    <nav 
      className="fixed left-0 right-0 mx-auto w-[calc(100%-2rem)] max-w-md bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm border border-gray-200/60 dark:border-gray-700/60 shadow-lg rounded-3xl flex justify-around py-1.5 z-[9999] pointer-events-auto transition-colors duration-200"
    ...
    >
      ...
      {appMode === 'jlpt' && (
        <>
          <Link href="/" className={`${navLinkClass} hover:text-indigo-600 dark:hover:text-indigo-300 ${isHome ? 'text-indigo-600 dark:text-indigo-300' : 'text-gray-500 dark:text-gray-400'}`}>
            <BookOpen size={20} />
            <span className="text-[10px] mt-0.5 font-medium">Home</span>
          </Link>
          <Link href="/library" className={`${navLinkClass} hover:text-orange-600 dark:hover:text-orange-300 ${isLibrary ? 'text-orange-600 dark:text-orange-300' : 'text-gray-500 dark:text-gray-400'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-star"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
            <span className="text-[10px] mt-0.5 font-medium">Library</span>
          </Link>
          <Link href="/settings" className={`${navLinkClass} hover:text-black dark:hover:text-white ${isSettings ? 'text-black dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>
