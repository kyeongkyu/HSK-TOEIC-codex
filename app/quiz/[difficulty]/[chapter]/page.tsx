...
export default function QuizPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`quiz-progress-${difficulty}-${chapterParam}`);
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (parsed && (parsed.currentIndex > 0 || parsed.isWrongReview)) {
  ...
  }, [difficulty, chapterParam]);

  useEffect(() => {
    if (quizState !== 'finished' && (currentIndex > 0 || isWrongReview)) {
      localStorage.setItem(`quiz-progress-${difficulty}-${chapterParam}`, JSON.stringify({
        currentIndex,
        isWrongReview,
      ...
      localStorage.removeItem(`quiz-progress-${difficulty}-${chapterParam}`);
    }
  }, [currentIndex, isWrongReview, wrongQueue, score, quizState, difficulty, chapterParam]);
  ...
  const quizWords = useMemo(() => {
    if (allFilteredWords.length === 0) return [];
    if (mode === 'topic') {
      let category = null;
      ...
      if (category) {
        return allFilteredWords.filter(w => category.words.includes(w.word));
      }
      return [];
    } else {
  ...
  }, [allFilteredWords, chapterId, chapterParam, mode]);
  ...
  const questionTypes = useMemo(() => {
    ...
    const types: ('hanzi-to-meaning' | 'meaning-to-hanzi')[] = new Array(quizWords.length).fill('hanzi-to-meaning');
  ...
  }, [currentWord, allFilteredWords, optionsCount]);
  ...
  if (!settingsLoaded || quizWords.length === 0) return <div className="p-8 text-center text-gray-500">Loading Quiz...</div>;
            ...
            <button
              onClick={() => {
                ...
                if (savedProgress) {
                  ...
                  setIsWrongReview(savedProgress.isWrongReview);
    ...
    return (
      <div className="p-6 min-h-screen bg-white dark:bg-gray-900 flex flex-col items-center justify-center text-center">
        <motion.div 
          ...
          className="w-full max-w-md bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded-[3rem] p-10 shadow-xl"
        >
          ...
          <h1 className="text-3xl font-black text-black dark:text-white mb-2">Quiz Finished!</h1>
          <p className="text-gray-500 dark:text-gray-400 font-medium mb-8 uppercase tracking-widest">
            {mode === 'topic' ? 'Topic Quiz' : `Chapter ${chapterId}`} • {difficulty}
          </p>
          
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
              <div className="text-4xl font-black text-blue-600 dark:text-blue-400 mb-1">{accuracy}%</div>
              <div className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Accuracy</div>
            </div>
            <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
              <div className="text-4xl font-black text-green-600 dark:text-green-400 mb-1">{score}/{quizWords.length}</div>
          ...
          <div className="space-y-3">
            ...
            {mode === 'chapter' && chapterId < chaptersCount && (
              <button 
                ...
                className="w-full py-4 bg-gray-100 dark:bg-gray-700 text-black dark:text-white rounded-2xl font-bold text-lg active:scale-95 transition-all flex items-center justify-center gap-2"
              >
            ...
            <button 
              ...
              className="w-full py-4 text-gray-500 dark:text-gray-400 font-bold hover:text-black dark:hover:text-white transition-colors"
            >
      ...
      <div className="h-32 flex items-center justify-center">
        {quizState === 'feedback' && (
          ...
          >
            ...
            <button 
              onClick={handleNext}
              className="w-full py-4 bg-black dark:bg-white text-white dark:text-black rounded-2xl font-bold text-lg shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2"
            >
